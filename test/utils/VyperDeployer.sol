...
contract VyperDeployer {
    ...
    /**
     * --optimizeStrategy {gas,codesize,none}. Optimization flag (defaults to 'gas')
     */
    function getDeployBytecodeBlueprint(string memory fileName, string memory evmVersion, string memory optimizeStrategy) public returns (bytes memory) {
        ///@notice create a list of strings with the commands necessary to compile Vyper contracts
    ...
    function deployBytecodeBlueprint(bytes memory deployBytecode) public returns (address) {
        ///@notice check that the deployment was successful
        address deployedAddress;
        assembly {
            deployedAddress := create(0, add(deployBytecode, 0x20), mload(deployBytecode))
