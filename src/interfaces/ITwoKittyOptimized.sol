...
interface ITwoKittyOptimized is IERC20 {
    ...
    // idx 0 is for prices, idx 1 is for xcp.
    ...
    /**
     * @dev Cached (fast to read) virtual price.
     * @dev The cached `virtual_price` is also used internally.
     */
    ...
    function nonces(address _address) external view returns (uint256);
     ...
     * @notice Pool will not call transferFrom and will only check if a surplus of
     * @notice coins[i] is greater than or equal to `dx`.
     * @dev Use-case is to reduce the number of redundant ERC20 token transfers in zaps. Primarily for dex-aggregators/arbitrageurs/searchers.
     * @dev Note for users: please transfer + exchange_received in 1 tx.
     * @param i Index value for the input coin
     * @param j Index value for the output coin
     * @param dx Amount of input coin being swapped in
     * @param min_dy Minimum amount of output coin to receive
