...
def initialise_ownership(_fee_receiver: address, _admin: address):
    ...
    assert msg.sender == self.deployer, "dev: deployer only"
    assert self.admin == empty(address), "dev: already initialised"
...
def deploy_pool(
    ...
    assert mid_fee < MAX_FEE-1, "Mid fee exceeds max fee"  # mid_fee can be zero
    assert out_fee >= mid_fee, "Out fee less than mid fee"
    assert out_fee < MAX_FEE-1, "Out fee exceeds max fee"
    assert fee_gamma < 10**18+1, "Fee gamma exceeds max"
    assert fee_gamma > 0, "Fee gamma cannot be zero"

    assert allowed_extra_profit < 10**18+1, "Allowed extra profit > max"

    assert adjustment_step < 10**18+1, "Adjustment step > max"
    assert adjustment_step > 0, "Adjustment step != be zero"

    assert ma_exp_time < 872542, "Ma exp time exceeds max"  # 7 * 24 * 60 * 60 / ln(2)
    assert ma_exp_time > 86, "Max exp time < 86"  # 60 / ln(2)

    assert initial_price > 10**6 and initial_price < 10**30, "11"  # dev: initial price out of bound
